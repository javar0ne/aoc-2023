...
public class Day4Second extends SecondQuestion {
    public Day4Second(Integer day) {
        super(day);
    }
    ...
    public Optional<Integer> solve() {
        ...
        Map<Long, Integer> scratchcardsNumber = new HashMap<>();
        ...
        for(String card : input) {
            ...
            scratchcardsNumber.put(
                cardNumber,
                scratchcardsNumber.getOrDefault(cardNumber, 0) + 1
            );

            String numbers = card.split(":")[1];

            List<Integer> winningNumbers = extractNumbers(numbers.split("\\|")[0]);
            List<Integer> numbersToMatch = extractNumbers(numbers.split("\\|")[1]);

            long numbersMatching = winningNumbers.stream()
                .filter(numbersToMatch::contains)
                .count();

            while(cardNumber <= input.size() && numbersMatching > 0) {
                long key = cardNumber + numbersMatching;
                scratchcardsNumber.put(
                    key,
                    scratchcardsNumber.getOrDefault(key, 0) + scratchcardsNumber.get(cardNumber)
                );
        ...
        int numOfScratchcards = scratchcardsNumber
            .values()
            .stream()
            .mapToInt(Integer::intValue)
            .sum();

        return Optional.of(numOfScratchcards);
    }

    private List<Integer> extractNumbers(String winningNumbers) {
        return Arrays.stream(winningNumbers.split(" "))
            .filter(winningNumber -> !winningNumber.isEmpty())
            .map(Integer::valueOf)
            .toList();
