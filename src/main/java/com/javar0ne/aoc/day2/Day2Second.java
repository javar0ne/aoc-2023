...
public class Day2Second extends SecondQuestion {

    private final static String RED = "red";
    private final static String GREEN = "green";
    private final static String BLUE = "blue";
    public Day2Second(Integer day) {
        super(day);
    }
    ...
    public Optional<Integer> solve() {
        ...
        int sum = input
            .stream()
            .mapToInt(this::handleGame)
            .sum();

        return Optional.of(sum);
    }

    private int handleGame(String line) {
        int minRedCubesNeeded = 0;
        int minGreenCubesNeeded = 0;
        int minBlueCubesNeeded = 0;
        String[] lineSplit = line.split(":");

        String[] games = lineSplit[1].split(";");

        for(String game : games) {
            String[] gameCubes = game.split(",");
            for (String gameCube : gameCubes) {
                int cubeNumber = Integer.parseInt(gameCube.split(" ")[1]);

                if(gameCube.contains(RED) && cubeNumber > minRedCubesNeeded) {
                    minRedCubesNeeded = cubeNumber;
                } else if(gameCube.contains(BLUE) && cubeNumber > minBlueCubesNeeded) {
                    minBlueCubesNeeded = cubeNumber;
                } else if(gameCube.contains(GREEN) && cubeNumber > minGreenCubesNeeded) {
                    minGreenCubesNeeded = cubeNumber;
                }

            }
        }

        return minRedCubesNeeded * minBlueCubesNeeded * minGreenCubesNeeded;
    }
}
